...
.PHONY: compile run jar test clean
...
jar: ./src/class/Main.class ./src/class/CredentialManager.class ./src/class/GitHubSearchTask.class ./src/class/Token.class
	...
	jar cvfm dorks.jar MANIFEST.MF -C src/class . -C src/lib .
	rm MANIFEST.MF
	...
	for thread in $$threads; do \
		for dorks in $$dorksFiles; do \
			for tokens in $$tokensFiles; do \
				for i in {1..15}; do \
					echo "Executando com $$thread threads, dorks: $$dorks, tokens: $$tokens (Execução $$i)"; \
